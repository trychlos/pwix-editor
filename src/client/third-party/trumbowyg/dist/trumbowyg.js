...
(function (navigator, window, document, $) {
    ...
    Trumbowyg.prototype = {
        ...
        // Destroy the editor
        // $c and $ed are the same element, actually the application-provided container
        destroy: function () {
			console.log( 'trumbowyg: entering destroy()', this );
			let res;
            var t = this,
                prefix = t.o.prefix;
			...
			//console.log( 'trumbowyg: before removeClass()' );
            $('body').removeClass(prefix + 'body-fullscreen');

			//console.log( 'trumbowyg: before trigger', t.$ed );
            t.$ed.trigger('tbwclose');

            $(window).off('scroll.' + t.eventNamespace + ' resize.' + t.eventNamespace);
            $(t.doc.body).off('keydown.' + t.eventNamespace);

			// verify we have an ad-hoc object
			console.log( 'trumbowyg: end of destroy()', this );
        },
        ...
        initPlugins: function () {
            ...
            $.each($.trumbowyg.plugins, function (name, plugin) {
                if (!plugin.shouldInit || plugin.shouldInit(t)) {
					...
					//console.log( 'trumbowyg: loading plugin', plugin );
                    t.loadedPlugins.push(plugin);
                }
            });
        },
        destroyPlugins: function () {
            console.debug( 'trumbowyg: entering destroyPlugins()' );
            var t = this;
            $.each(this.loadedPlugins, function (i, plugin) {
                if (plugin.destroy) {
					// seems that some plugins have difficulties to destroy themselves
	                plugin.destroy(t);
                }
				return true;
            });
            console.debug( 'trumbowyg: returning from destroyPlugins()' );
        }
    };
