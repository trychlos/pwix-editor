 ...
 *
 *  Usage:
 *  - the plugin must be first, and unless otherwise stated, called on the TABLE element
...
;( function( $, window, document ){
    ...
    $.extend( myPlugin.prototype, {
        ...
        _create(){
            this.$dom.addClass( 'itfx-igrid-table' );
        ...
        _installSortBtn( idx, $column ){
            ...
            $column.empty();
            $column.append( ''+
                '<div class="itfx-igrid-th d-flex flex-row justify-content-start align-items-center">'+
                    '<div class="itfx-igrid-name">'+text+'</div>'+
                    '<div class="itfx-igrid-sort-btn">'+
                        '<span class="'+this.settings.sort.icon.font.none+' '+this.settings.sort.icon.span+'"></span'+
                    '</div>'+
                '</div>'
            );
            $column.data( 'itfx-igrid-idx', idx );
            $column.find( '.itfx-igrid-sort-btn' ).attr( 'igrid-sort', 'none' );
            $column.find( '.itfx-igrid-sort-btn' ).click( this, function( ev ){
                const self = this;  // the .itfx-igrid-sort-btn DIV DOM element
                const plugin = ev.data;
                // get the initial value
                const prev = $( this ).attr( 'igrid-sort' );
                // reinit all sort buttons
                $( $( this ).parents( 'thead' )[0] ).find( 'th .itfx-igrid-sort-btn' ).each( function(){
                    $( this )
                        .attr( 'igrid-sort', 'none' )
                        .find( '>svg' )
                            .removeClass()
                            .addClass( plugin.settings.sort.icon.font.none+' '+plugin.settings.sort.icon.svg );
                });
                ...
                if( next ){
                    $( self )
                        ...
                        .find( '>svg' )
                            .removeClass()
                            .addClass( plugin.settings.sort.icon.font[next]+' '+plugin.settings.sort.icon.svg );
                }
                ...
                const idx = $( th ).data( 'itfx-igrid-idx' );
                const asc = ( next === 'up' ? 1 : -1 );
                // does the current field qualified by 'data-column-name' provide a compare function ?
                const _providedFn = () => {
