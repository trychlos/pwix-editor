...
Meteor.methods({
    ...
    'te_contents.empty'( collection_name ){
        ...
        logger.verbose({ verbosity: Editor.configure().verbosity, against: Editor.C.Verbose.COLLECTIONS }, 'te_contents.empty() called with collection='+collection_name );
        ...
        logger.verbose({ verbosity: Editor.configure().verbosity, against: Editor.C.Verbose.COLLECTIONS }, 'te_contents.empty() returns with', res );
        return res;
    },
    ...
    'te_contents.byName'( collection_name, document_name ){
        ...
        logger.verbose({ verbosity: Editor.configure().verbosity, against: Editor.C.Verbose.COLLECTIONS }, 'te_contents.byName() called with collection='+collection_name, 'document='+document_name );

        const collection = Editor.collections.get( collection_name, Editor.collections.Contents.schema );
        const res = collection.findOneAsync({ name: document_name });

        logger.verbose({ verbosity: Editor.configure().verbosity, against: Editor.C.Verbose.COLLECTIONS }, 'te_contents.byName() returns with', res );
        return res;
    },
    ...
    'te_contents.import'( collection_name, elt ){
        ...
        logger.verbose({ verbosity: Editor.configure().verbosity, against: Editor.C.Verbose.COLLECTIONS }, 'te_contents.import() called with elt=', elt );

        const collection = Editor.collections.get( collection_name, Editor.collections.Contents.schema );
        const res = collection.insertAsync( elt );

        logger.verbose({ verbosity: Editor.configure().verbosity, against: Editor.C.Verbose.COLLECTIONS }, 'te_contents.import() returns with', res );
        return res;
    },
    ...
    'te_contents.set'( collection_name, document_name, content ){
        assert( collection_name && _.isString( collection_name ) && collection_name.length, 'te_contents.byName expects collection_name be a non-empty string' );
        assert( document_name && _.isString( document_name ) && document_name.length, 'te_contents.byName expects document_name be a non-empty string' );
        logger.verbose({ verbosity: Editor.configure().verbosity, against: Editor.C.Verbose.COLLECTIONS }, 'te_contents.set() called with collection='+collection_name, 'document='+document_name, 'content=', content );
        ...
        return collection.findOneAsync({ name: document_name })
            ...
            .then(( res ) => {
                logger.verbose({ verbosity: Editor.configure().verbosity, against: Editor.C.Verbose.COLLECTIONS }, 'te_contents.set() returns with', res );
            });
    },
